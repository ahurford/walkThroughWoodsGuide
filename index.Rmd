...
Be sure to type exactly `A.mm` after the `$`. Try making your plot:
...
8.  Can you make a [plot](https://ahurford.github.io/quant-guide-all-courses/graph.html#scatter-plots-with-plotxy) of different variables, for example, `Wood density` and `Average of the absolute value of latitude`?
...
10. To analyze the relation between variables we can use the `cor(x,y,method = "pearson")` function. See [Correlation](https://ahurford.github.io/quant-guide-all-courses/statistics.html#correlation) for details.
...
2.  We would like to perform a t-test to compare the means of the average cross-sectional vessel area for Rhamnaceae and Fabaceae, however, before we do this we will graph these data to observe if we expect the t-test to find a significant different in means.
...
(Note `tail` gives the last 6 rows).

**Approach 2: dplyr**

In `dplyr`, can use make the data frame, `data`, by using `select()` to choose columns, and filter to keep the families of interest. We can do this efficiently using pipes:
...
If we have made a dataframe with the correct structure, then we can use the `boxplot` command to visualize the average cross sectional area for vessels of plants from the families `Rhamnaceae` and  `Fabaceae`.
